$("#container li").each(function(){
   ...
   function mouseEvent(angle,o,d){ //方向  元素  鼠标进入/离开
	   ...
	   if(d=='in'){
		   switch(angle){
			   case '上':
				   $(o).find(".newp").css({left:0,top:-h+"px"}).stop(true).animate({left:0,top:0},300)
					
				   break;
			   case '右':
				   $(o).find(".newp").css({left:w+"px",top:0}).stop(true).animate({left:0,top:0},300)
		   ...
		   switch(angle){
			   case '上':
				   ...
				   setTimeout(function(){
					   $(o).find(".newp").stop(true).animate({left:0,top:-h+"px"},300)
				   },200)
				   break;
			   case '右':
				  
				   setTimeout(function(){
					   $(o).find(".newp").stop(true).animate({left:w+"px",top:0},300)
				   },200)
				   break;
			   case '下':
