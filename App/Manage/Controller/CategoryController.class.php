...
class CategoryController extends CommonController {
	...
	public function addPost() {
		...
		$data['image'] = I('image', '', 'htmlspecialchars,trim');
		$data['des'] = I('des', '', '');
		...
		$rr = M('category')->where(array('ename'=>$data['ename']))->find();
		if(!empty($rr)){
			$this->error('别名不能重复！');
		}
	...
	public function set_pinyin(){
		$cate = get_category(10);
		foreach($cate as $k=>$val){
			$pinyin = get_pinyin(iconv('utf-8', 'GBK//ignore', $val('name')), 0, 1, C('DEFAULT_LANG'));
			$data['id'] = $val['id'];
			$data['ename'] = $pinyin;
			$aa = M('category')->where(array('ename'=>$pinyin))->find();
			if(empty($aa)){
				M('category')->save($data);
			}
		}
