...
str;
	...
	public function sendCode2() {
		...
		if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
			//exit(json_encode(array('status'=>0,'info'=>'E-mail格式不正确！','input'=>'email')));
			//$this->ajaxReturn(array('status'=>0,'info'=>'E-mail格式不正确！','input'=>'email'),'JSON');//Thinkphp内部
			$this->error('E-mail format is incorrect!', '', array('input' => 'email')); //TP3.1后，error和success支持ajax返回

		}
		
		$data['code']    = get_random(6, '1234567890');

		session(md5($email),$data['code']);
		...
		$subject = "[{$webname}] join GDIO verification code";
		$message = <<<str
<p>The verification code of your application is：{$data['code']}</p>
<p> </p>
<p>1.In order to ensure your safety, please do not tell anyone the above verification code, our staff will not ask you for the verification code.</p>
<p>2. If this verification code is not your application, please ignore this email.</p>
<p>This email is sent by the system. Please do not reply directly.</p>
str;
		$msg = '';
		if (send_mail($email, $subject, $message) == true) {
			$msg .= ''; //'验证邮件已发送，请尽快查收邮件，激活该帐号';
		...
		$this->success('The verification code was sent successfully. Please check it in the email' . $msg, '', array('input' => 'email'));
