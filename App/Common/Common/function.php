...
//短信接口
function sendmessage($phone='',$content='',$sendtime=''){
	header("Content-type:text/html; Charset=utf-8");
	$post_data = array();
	$post_data['userid'] = C('CFG_MESSAGE_USERID');
	$post_data['account'] = C('CFG_MESSAGE_USERNAME');
	$post_data['password']= C('CFG_MESSAGE_PASSWORD');
	$post_data['content'] =$content;//."【".C('CFG_WEBNAME')."】"; 
	...
	$resault = trim($resault);
	$doc = DOMDocument::loadXML($resault);
	$returnstatus = $doc->getElementsByTagName( "returnstatus" ); //取得humans标签的对象数组
	$status = $returnstatus->item(0)->nodeValue;	
	if($status == 'Success'){
		return true;
	}else{
		return false;
