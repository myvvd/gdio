...
str;
	...
	public function _prev($attr, $content) {
		$titlelen = empty($attr['titlelen']) ? 0 : intval($attr['titlelen']);
		$notitle  = isset($attr['notitle']) ? trim($attr['notitle']) : 'first';
...
<?php
	...
	if(empty(\$content['id']) || empty(\$content['cid']) || empty(\$cate['table_name']) ) {
		...
		//上一条记录
        \$_vo= D2('ArcView', \$cate['table_name'])->where(array(\$cate['table_name'].'.delete_status' => 0, 'cid' => \$content['cid'], 'id' => array('lt',\$content['id'])))->order('point desc,id desc')->find();
...
str;
	...
	public function _next($attr, $content) {
		$titlelen = empty($attr['titlelen']) ? 0 : intval($attr['titlelen']);
		$notitle  = isset($attr['notitle']) ? trim($attr['notitle']) : 'last';
		$target   = isset($attr['target']) ? trim($attr['target']) : '';
		$target   = empty($target) ? '' : ' target="' . $target . '"';
		$str      = <<<str
<?php
	if(empty(\$content['id']) || empty(\$content['cid']) || empty(\$cate['table_name']) ) {
		...
		//下一条记录
        \$_vo= D2('ArcView',\$cate['table_name'])->where(array(\$cate['table_name'].'.delete_status' => 0, 'cid' => \$content['cid'], 'id' => array('gt',\$content['id'])))->order('point asc,id ASC')->find();
